...
test.describe('Form layout tests', () => {
    ...
    expect.soft(signInBtn).toHaveText('Sign in2');
    signInBtn.click();
    ...
    // const emailPaceholderValue = await emailField.getAttribute('placeholder');
    // expect(emailPaceholderValue).toEqual('Email');

    // const radioBtns = await gridForm.locator('mb-radio').allTextContents();
  ...
  test('different assertions', async({page}) => {
    ...
    const gridForm = page
			...
			.filter({ hasText: 'Using the Grid' });
		const signInBtn = gridForm.getByRole('button');
		expect.soft(signInBtn).toHaveText('Sign in');
	});
  });
